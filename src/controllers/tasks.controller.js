...
const getTasksByStatusOrDate = async (req, res) => {
    ...
    try {
        ...
        if (tasks.length === 0) {
            return res.status(404).json({ message: 'Nenhuma tarefa encontrada com os critérios de busca fornecidos.' });
        }
        return res.status(200).json(tasks);
    } catch (error) {
        return res.status(500).json({ message: error.message });
...
const updateTask = async (req, res) => {
    ...
    try {
        ...
        if (taskWithSameTitle && String(taskWithSameTitle._id) != id) {
            return res.status(400).json({ message: 'Já existe uma tarefa com este título!' });
        }

        const task = await Task.findByIdAndUpdate(id, {title, description, status, updatedAt: format(new Date(), 'dd/MM/yyyy HH:mm:ss') }, { new: true });
